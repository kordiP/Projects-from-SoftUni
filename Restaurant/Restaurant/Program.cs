...
namespace Resturant
{
    class Program
    {
        ...
        private static void AddMealProducts(string name, string type, int productsCount)
        {
            ...
            try
            {
                Meal meal = new Meal(name, type, mealProducts);
                meals.Add(name, meal);
            }
            catch (ArgumentException e)
            {
        ...
        private static void GetMealPrice(string mealName)
        {
            ...
            {
                Console.WriteLine(Math.Round((double)meals[mealName].GetType().GetProperty("Price").GetValue(meals[mealName]), 2));
                Console.WriteLine("The price of {0} is: {1}", mealName , Math.Round((double)meals[mealName].GetType().GetProperty("Price").GetValue(meals[mealName]), 2));
            }
        }
    ...
    class Meal
    {
        ...
        public double Price
        {
            get
            {
                tempPrice = products.Sum(x => x.Price);
                tempPrice *= 1.3;
                return tempPrice;
            }
        }
        ...
        //{
        //    Meal mostPopularMeal = meals.OrderByDescending(x => x.Value.Ordered).First();
