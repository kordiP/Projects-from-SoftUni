...
namespace arraysExcercise
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int[] lBugInitialPositionIndex = Console.ReadLine().Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();
            ...
            while (command != "end")
            {
                string[] rules = command.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).ToArray();
                ...
                switch (direction)
                {
                    case "right":
                        ...
                        {
                            for (int i = newPosition; i < ladyBugField.Length; i += countOfMoves) // array goes till LB find free index, or goes out of the field
                            {
